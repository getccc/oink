#!/usr/bin/env python3
"""Build and verify PRD 4 local-search metadata, ranking, and budgets."""
...
CONTRACT_PATH = ROOT / "tests" / "fixtures" / "prd4" / "contract.json"
...
def validate_language(
    ...
    excluded_suffixes = (
        "/docs/excluded-canonical/",
        "/docs/excluded-legacy/",
        "/docs/excluded-camel/",
    )
    for suffix in excluded_suffixes:
        require(
            not any(doc["ref"].endswith(suffix) for doc in documents),
            f"{lang} index retained excluded alias fixture {suffix}",
        )
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="oink-prd4-search-") as temp:
            workspace = Path(temp)
            ...
            for deployment, indexes in built_indexes.items():
                behavior = subprocess.run(
                    [
                        "node",
                        str(ROOT / "tests" / "js" / "prd4-search-engine.test.js"),
                        str(indexes["en"]),
                        str(indexes["zh"]),
                    ],
                    cwd=ROOT,
        ...
        print(f"PRD 4 search check failed: {exc}", file=sys.stderr)
        return 1

    print("PRD 4 search metadata and ranking checks passed")
